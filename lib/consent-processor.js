...
async function processDecision(consentsBundle, query) {
  const consentDecisionPromises = consentsBundle
    ...
    .filter(entry => matchesPurpose(entry, query))
...
function matchesPurpose(entry, query) {
  const untrimmedPurposes = _.get(entry, "resource.provision.purpose") || [];
  ...
  return (
    !query.purposeOfUse ||
    !_.get(entry, "resource.provision.purpose") ||
